...
Vec3f scene::rayTrace(Vec3f eye, Vec3f dir, int recurseDepth)
{
	...
	else
	{
		...
		Vec3f coords = myObjGroup->getClosest()->getTextureCoords(eye, hitPoint);
		...
		textureColor = textureColor*((float)1/255.0);
	}

	//add diffuse color times ambient light to our answer
	Vec3f  diffuse = myMaterials.at(matIndex).diffuseCol;
	answer += (multiplyColorVectors(ambLight, diffuse));
	...
	for( size_t i = 0; i < myLights.size(); ++i)
	{
		...
		Vec3f lightDir = hitPoint - lt.position;
		lightDir.Normalize();

		float lightDist = myObjGroup->getClosest()->testIntersection(lt.position, lightDir);

		if(lightDist == MISS)
			continue;

		int index = myObjGroup->getClosest()->getMatIndex();
		Vec3f reflect = myMaterials.at(index).reflectiveCol;
		reflect.Scale(0.5,0.5,0.5);
		float shine = myMaterials.at(index).shininess;

		float normDotLight = normal.Dot3(lightDir);

		Vec3f diff = diffuse;
		diff.Scale(normDotLight,normDotLight,normDotLight);
		
		float reflectDotView = pow(reflect.Dot3(lightDir), shine);

		float t = normDotLight + reflectDotView;

		Vec3f light = lt.color;
		light.Scale(t,t,t);
		totalLight += light;
	}

	//add the diffuse light times the accumulated diffuse light to our answer
	answer += multiplyColorVectors(diffuse, totalLight);
	...
	if (recurseDepth<3)
	{
		...
		Vec3f reflection;
		Vec3f temp = normal;
		float c1 = 2.0f * (normal.Dot3(dir));
		temp.Scale(c1, c1, c1);
		reflection = dir - temp;
		reflection.Normalize();

		//recusively raytrace from the surface point along the reflected view
		Vec3f reflectionAnswer = rayTrace(hitPoint,reflection,recurseDepth+1);
		
		//add the color seen times the reflective color
		answer  += multiplyColorVectors(myMaterials.at(matIndex).reflectiveCol, reflectionAnswer);
...
Vec3f scene::multiplyColorVectors(Vec3f a, Vec3f b) 
{
